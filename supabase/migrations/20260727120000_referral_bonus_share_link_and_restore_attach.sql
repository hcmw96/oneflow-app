-- Fix refer-a-friend Flow Points:
-- 1) Restore referrals-row creation when email class-invites attach on signup
--    (regressed in 20260618160000_class_invites_paid_completion).
-- 2) Award referrer on the referred member's FIRST non-cancelled booking for
--    both email invites AND share links (?ref= → profiles.referred_by).
-- Idempotent: referrals.points_awarded must be 0; cancel+rebook does not re-award.

-- ---------------------------------------------------------------------------
-- Manual verification (run in SQL editor after a test booking):
--
--   -- Referrer balance + ledger
--   select id, first_name, flow_points
--   from profiles where id = '<referrer_uuid>';
--
--   select * from flow_points
--   where profile_id = '<referrer_uuid>' and reason = 'referral_bonus'
--   order by created_at desc limit 5;
--
--   select * from referrals
--   where referred_id = '<friend_uuid>';
--
--   -- Cancel friend booking and rebook: points_awarded stays 500, no second ledger row.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- First non-cancelled booking only (cancel + rebook: earlier award already set points_awarded).
  if exists (
    select 1
    from public.bookings b
  ...
  -- Prefer an unpaid referrals row (email invite or share link).
  select r.id, r.referrer_id, r.referral_type
    into v_referral_id, v_referrer_id, v_referral_type
    from public.referrals r
    where r.referred_id = new.profile_id
      and coalesce(r.points_awarded, 0) = 0
      and r.referral_type in ('class_invite_email', 'share_link')
    order by r.created_at
    limit 1;

  -- Fallback: booking linked to an email class-invite.
  if v_referrer_id is null and new.class_invite_id is not null then
    ...
    if v_referrer_id is not null then
      v_referral_type := 'class_invite_email';
      insert into public.referrals (referrer_id, referred_id, referral_type, points_awarded)
  ...
  -- Fallback: share link (?ref=) stored on profiles.referred_by.
  if v_referrer_id is null then
    select p.referred_by
      ...
      where p.id = new.profile_id
        ...
        and p.referred_by <> new.profile_id;

    if v_referrer_id is not null then
      v_referral_type := 'share_link';
      insert into public.referrals (referrer_id, referred_id, referral_type, points_awarded)
      select v_referrer_id, new.profile_id, 'share_link', 0
      where not exists (
        select 1
        from public.referrals r
        where r.referrer_id = v_referrer_id
  ...
  -- Already paid out for this pair.
  if exists (
    select 1 from public.referrals r
    where r.id = v_referral_id and coalesce(r.points_awarded, 0) > 0
  ) then
    return new;
  end if;
