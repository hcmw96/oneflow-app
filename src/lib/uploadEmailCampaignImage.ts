...
/** Upload a campaign inline image; returns a public HTTPS URL for use in email HTML. */
export async function uploadEmailCampaignImage(file: File): Promise<string> {
  ...
  const blob = await compressImageIfNeeded(file, 800_000);
